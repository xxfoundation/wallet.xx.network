...
function createRmProxy (api: ApiPromise, account: AccountId, type: XxnetworkRuntimeProxyType, delay = 0): SubmittableExtrinsic<'promise'> {
...
function getProxyTypeInstance (api: ApiPromise, index = 0): XxnetworkRuntimeProxyType {
  ...
  return api.createType<XxnetworkRuntimeProxyType>(proxyNames.length ? proxyNames[0] : 'ProxyType', index);
}
...
function ProxyOverview ({ className, onClose, previousProxy: [existing] = EMPTY_EXISTING, proxiedAccount }: Props): React.ReactElement<Props> {
  ...
  const _delPrev = useCallback(
    (accountId: AccountId, type: XxnetworkRuntimeProxyType, index: number): void => {
