...
function expandInfo ({ exposure, commission, pastAvgCommission, teamMultiplier }: ValidatorInfo): StakingState {
  ...
  return {
    commission: commission?.unwrap().toHuman(),
    nominators,
    stakeOther,
    stakeOwn,
    stakeTotal,
...
function Address ({ address, className = '', filterName, hasQueries, isElected, isFavorite, isMain, isPara, lastBlock, nominatedBy, points, recentlyOnline, toggleFavorite, validatorInfo, withIdentity }: Props): React.ReactElement<Props> | null {
  ...
  const { commission, nominators, stakeOther, stakeOwn, teamMultiplier, pastAvgCommission } = useMemo(
    () => validatorInfo
