...
function isValidNumber(bn: BN, bitLength: BitLength, isZeroable: boolean, minValue?: BN, maxValue?: BN): boolean {
  if (
    // cannot be negative
    bn.lt(BN_ZERO) ||
...
function inputToBn(api: ApiPromise, input: string, si: SiDef | null, bitLength: BitLength, isZeroable: boolean, minValue?: BN, maxValue?: BN, decimals?: number): [BN, boolean] {
  ...
  return [
    result,
    isValidNumber(result, bitLength, isZeroable, minValue, maxValue)
  ];
}

function getValuesFromString(api: ApiPromise, value: string, si: SiDef | null, bitLength: BitLength, isZeroable: boolean, minValue?: BN, maxValue?: BN, decimals?: number): [string, BN, boolean] {
  const [valueBn, isValid] = inputToBn(api, value, si, bitLength, isZeroable, minValue, maxValue, decimals);
...
function getValues(api: ApiPromise, value: BN | string = BN_ZERO, si: SiDef | null, bitLength: BitLength, isZeroable: boolean, minValue?: BN, maxValue?: BN, decimals?: number): [string, BN, boolean] {
  return isBn(value)
    ? getValuesFromBn(value, si, isZeroable, decimals)
    : getValuesFromString(api, value, si, bitLength, isZeroable, minValue, maxValue, decimals);
}

function InputNumber({ autoFocus, bitLength = DEFAULT_BITLENGTH, children, className = '', defaultValue, help, isDecimal, isFull, isSi, isDisabled, isError = false, isWarning, isZeroable = true, label, labelExtra, maxLength, minValue, maxValue, onChange, onEnter, onEscape, placeholder, siDecimals, siDefault, siSymbol, value: propsValue, commission }: Props): React.ReactElement<Props> {
  ...
  const [[value, valueBn, isValid], setValues] = useState<[string, BN, boolean]>(() =>
    getValues(api, propsValue || minValue, si, bitLength, isZeroable, minValue, maxValue, siDecimals)
  );
  ...
  const _onChangeWithSi = useCallback(
    (input: string, si: SiDef | null) => setValues(
      getValuesFromString(api, input, si, bitLength, isZeroable, minValue, maxValue, siDecimals)
    ),
    [api, bitLength, isZeroable, isDecimal, minValue, maxValue, siDecimals]
  );
  ...
  let maxValueLength = getGlobalMaxValue(bitLength).toString().length;
